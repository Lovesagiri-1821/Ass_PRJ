...
<div class="main-content">
    <h2>✏️ Edit Image (ID: ${image.imageId})</h2>

    <form action="ImageController" method="post">
        <input type="hidden" name="action" value="update" />
        <input type="hidden" name="imageId" value="${image.imageId}" />
        <input type="hidden" name="propertyId" value="${image.propertyId}" />

        <label>🖼️ Image URL:</label>
        <input type="text" name="imageUrl" value="${image.imageUrl}" required />

        <button type="submit">💾 Update</button>
    </form>

    <a class="back-link" href="ImageController?action=view&propertyId=${image.propertyId}">⬅️ Back to image list</a>
</div>
